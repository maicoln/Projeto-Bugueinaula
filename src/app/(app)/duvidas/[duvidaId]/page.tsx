...
// Tipos "Limpos" que o nosso componente vai usar
...
type Resposta = {
  ...
  votes: number;
  profile: ProfileInfo | null; // Objeto único
...
// Tipos "Brutos" (Raw) que correspondem à resposta da API do Supabase
...
type RawResposta = {
  ...
  is_best_answer: boolean;
  votes: number;
  profiles: RawProfile[] | null; // Supabase retorna como array
...
export default function DuvidaDetalhePage() {
  ...
  useEffect(() => {
    async function getUser() {
      ...
      if (user) {
        ...
        if (profile) setCurrentUser(profile as { id: string; tipo_usuario: string });
      }
    }
    void getUser();
  }, []);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', 
        { event: '*', schema: 'public', table: 'respostas_duvidas', filter: `duvida_id=eq.${duvidaId}` },
        () => { void mutate(swrKey); }
      ).on('postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'duvidas', filter: `id=eq.${duvidaId}` },
        () => { void mutate(swrKey); }
      ).subscribe();
  ...
  const handlePostAnswer = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('respostas_duvidas').insert({ corpo: newAnswer, duvida_id: Number(duvidaId), autor_id: currentUser.id });
  ...
  return (
    <div className="p-6 animate-fade-in max-w-4xl mx-auto">
        ...
        <div className="p-6 rounded-lg border bg-white dark:bg-gray-800 dark:border-gray-700">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Perguntado por {duvida.profile?.nome || 'Anónimo'} em {new Date(duvida.created_at).toLocaleDateString('pt-BR')}</p>
        ...
        <div className="space-y-4">
            {sortedRespostas.map(resposta => (
                <div key={resposta.id} className={`rounded-lg border p-5 ${resposta.is_best_answer ? 'bg-green-50 border-green-300 dark:bg-green-900/20 dark:border-green-700' : 'bg-white dark:bg-gray-800 dark:border-gray-700'}`}>
                    <div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className={`font-semibold ${resposta.profile?.tipo_usuario === 'PROFESSOR' ? 'text-blue-600' : 'text-gray-800 dark:text-gray-200'}`}>{resposta.profile?.nome}</span>{resposta.profile?.tipo_usuario === 'PROFESSOR' && <span className="text-xs font-bold text-white bg-blue-600 px-2 py-0.5 rounded-full">PROFESSOR</span>}</div><span className="text-xs text-gray-500">{new Date(resposta.created_at).toLocaleString('pt-BR')}</span></div>
                    ...
                    <div className="mt-4 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <button onClick={() => handleUpvote(resposta.id)} disabled={resposta.user_has_voted} className={`flex items-center gap-1.5 text-sm transition ${resposta.user_has_voted ? 'text-blue-600 font-bold' : 'text-gray-500 hover:text-blue-600'}`}><ThumbsUp size={16} /> {resposta.votes}</button>
